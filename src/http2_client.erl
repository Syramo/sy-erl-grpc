...
% -define(INITIAL_WINDOW_SIZE, 131072).
...
-spec new_stream(Connection::pid(),
...
-spec send_headers(Connection::pid(), stream_id(), [header()],
...
-spec stream_window_update(Connection::pid(), stream_id(), Increment::integer()) ->
    ok | {error, term()}.
...
init({Transport0, Host, Port, Options, Owner}) ->
    ...
    ConnectResult = Transport:connect(Host, Port,
                                      [{inet_backend, socket}] ++ TransportOptions ++ default_opts(Transport)),
...
-spec handle_call(term(), any(), connection()) ->
...
terminate(_Reason, #{keep_alive := KeepAlive}) ->
    stop_ping(KeepAlive),
    ok.

stop_ping (false) -> ok;
stop_ping (Tref) -> timer:cancel(Tref).
...
new_connection(Connection, Options) ->
    ...
    case settings_frame(NonDefaultSettings) of
        {ok, Frame} ->
            KeepAlive = case maps:get(keep_alive, NonDefaultSettings, 30000) of
                N when is_integer(N) -> timer:apply_interval(N,?MODULE,ping,[self(),1000]);
                _ -> false
            end,
            send_settings(Frame, 
...
send_data_frame(Data, MaxFrameSize, Flags, Connection, Stream, Size)
  when Size =< MaxFrameSize ->
    %%io:format("Bytes on wire ~B / ~B ~n",[byte_size(Data), Size]),
...
close_connection(#{transport := Transport, socket := Socket} = C) ->
    io:format("got close connection request: ~n"),
...
default_opts(ssl) ->
    [{packet, raw}, {active, false}, binary, {alpn_advertised_protocols,[<<"h2">>]}, {log_level, none}];
default_opts(gen_tcp) ->
    [{packet, raw}, {active, false}, binary, {buffer, 128000}];
default_opts(_) -> [].
